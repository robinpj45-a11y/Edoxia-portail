...
export default function SuccessEvaluationCreator() {
  ...
  useEffect(() => {
    ...
    const fetchClasses = async () => {
      const q = query(collection(db, 'test_success_classes'), where('spaceId', '==', spaceId));
    ...
    const fetchEditData = async () => {
      if (editId) {
        try {
          const snap = await getDoc(doc(db, 'test_success_evaluations', editId));
          if (snap.exists()) {
    ...
    try {
      ...
      if (status === 'published' && (!students || students.length === 0)) {
        const studentsSnap = await getDocs(query(
          collection(db, 'test_success_students'), 
      ...
      if (editId) {
        await setDoc(doc(db, 'test_success_evaluations', editId), data);
      } else {
        const docRef = await addDoc(collection(db, 'test_success_evaluations'), data);
        if (status === 'published') {
