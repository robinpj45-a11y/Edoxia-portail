...
export default function SuccessEvaluationDetail() {
  ...
  useEffect(() => {
    const unsubEval = onSnapshot(doc(db, 'test_success_evaluations', evalId), (snap) => {
      if (snap.exists()) {
    ...
    const resultsRef = doc(db, 'test_success_results', evalId);
  ...
  const saveResults = async () => {
    setSaving(true);
    try {
      await setDoc(doc(db, 'test_success_results', evalId), {
        data: results,
        spaceId,
        updatedAt: new Date()
      });
  ...
  const handleDelete = async () => {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer cette évaluation et tous ses résultats ? Cette action est irréversible.")) {
      try {
        await deleteDoc(doc(db, 'test_success_evaluations', evalId));
        await deleteDoc(doc(db, 'test_success_results', evalId));
        navigate(`/success/${spaceId}`);
      } catch (e) {
