...
export default function SuccessSpacePage() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      // Fetch evaluations
      const qEvals = query(collection(db, 'test_success_evaluations'), where('spaceId', '==', spaceId));
      ...
      // Fetch classes
      const qClasses = query(collection(db, 'test_success_classes'), where('spaceId', '==', spaceId));
      const unsubscribeClasses = onSnapshot(qClasses, async (snapshot) => {
        ...
        if (cls.length === 0) {
          try {
            ...
            const defaultCls = await addDoc(collection(db, 'test_success_classes'), {
              name: defaultName,
              spaceId,
            ...
            if (spaceId === 'demo') {
              ...
              for (const s of demoStudents) {
                await addDoc(collection(db, 'test_success_students'), {
                  ...s,
      ...
      <div className="flex flex-col h-screen bg-brand-bg items-center justify-center p-6 text-brand-text">
        ...
        >
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            ...
            <button type="button" onClick={() => navigate('/test-success')} className="text-sm font-bold text-brand-text/40 hover:text-brand-coral uppercase tracking-widest mt-4">
              Retour au Hub
            </button>
          </form>
        </motion.div>
  ...
  return (
    <div className="flex flex-col h-screen bg-brand-bg relative overflow-hidden text-brand-text pb-24 md:pb-0">
      ...
      <header className="shrink-0 sticky top-0 z-[60] border-b border-white/50 p-4 px-8 hidden md:flex justify-between items-center shadow-soft bg-white/40 backdrop-blur-md">
        <button onClick={() => navigate('/test-success')} className="flex items-center gap-2 transition-colors font-bold text-brand-text/50 hover:text-brand-text">
