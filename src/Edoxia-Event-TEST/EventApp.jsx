...
export default function EventApp({ user: propUser }) {
  ...
  useEffect(() => {
    ...
    const qEvents = query(collection(db, 'test_events'), orderBy('date', 'asc'));
    ...
    const qEntries = query(collection(db, 'test_entries'), orderBy('timestamp', 'desc'));
    ...
    if (!loading && events.length > 0) {
      if (eventId) {
        ...
        if (found) {
          ...
          navigate('/test-events', { replace: true });
        }
      } else {
