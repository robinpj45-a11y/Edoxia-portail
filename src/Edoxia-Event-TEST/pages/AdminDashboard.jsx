...
function AdminOverview({ events, entries, onEdit }) {
  ...
  const deleteEvent = async (id) => { if (confirm("Supprimer ?")) await deleteDoc(doc(db, 'test_events', id)); };

  const togglePaid = async (entryId, currentStatus) => {
    try { await updateDoc(doc(db, 'test_entries', entryId), { isPaid: !currentStatus }); }
    catch (err) { alert("Erreur mise à jour paiement"); }
  };

  const toggleEventLock = async (event) => {
    try { await updateDoc(doc(db, 'test_events', event.id), { isLocked: !event.isLocked }); }
...
function AdminCreateForm({ eventToEdit, user, onFinish }) {
  ...
  const save = async () => {
    ...
    if (eventToEdit) await updateDoc(doc(db, 'test_events', eventToEdit.id), data);
    else await addDoc(collection(db, 'test_events'), { ...data, createdAt: serverTimestamp(), creatorId: user.uid });
    onFinish();
  };
