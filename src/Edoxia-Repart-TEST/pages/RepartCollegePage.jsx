...
export default function RepartCollegePage() {
  ...
  useEffect(() => {
    if (historyModalOpen) {
      const fetchHistory = async () => {
        try {
          const q = query(collection(db, "test_repart_history"), orderBy("timestamp", "desc"));
  ...
  useEffect(() => {
    ...
    teams.forEach(async (team) => {
      if (defaultGroupsToDelete.includes(team.name.toLowerCase())) {
        try {
          await deleteDoc(doc(db, "test_repart_classes_college", team.id));
        } catch (e) {
  ...
  const handleAddStudent = async (e) => {
    ...
    try {
      await addDoc(collection(db, "test_repart_students"), {
        name,
        lastName,
        firstName,
  ...
  const handleAddClass = async (e) => {
    ...
    try {
      await addDoc(collection(db, "test_repart_classes_college"), {
  ...
  const handleDeleteClass = async (teamId, numId) => {
    ...
    try {
      await deleteDoc(doc(db, "test_repart_classes_college", teamId));
      const updates = students.filter(s => s.team === numId).map(s => 
        updateDoc(doc(db, "test_repart_students", s.id), { team: null })
      );
  ...
  const toggleClassLock = async (teamId, currentLocked) => {
    try {
      await updateDoc(doc(db, "test_repart_classes_college", teamId), { locked: !currentLocked });
    } catch (error) { console.error(error); }
  };
  ...
  const toggleAttribute = useCallback(async (id, attribute) => {
    ...
    await updateDoc(doc(db, "test_repart_students", id), { [attribute]: !student[attribute] });
  }, [students, teams]);
  ...
  const toggleStatus = useCallback(async (id, status) => {
    ...
    await updateDoc(doc(db, "test_repart_students", id), { status: newStatus });
  }, [students, teams]);
  ...
  const handleDeleteStudent = useCallback(async (id) => {
    ...
    if (confirm("Attention : cela supprimera définitivement l'élève de la base. Continuer ?")) await deleteDoc(doc(db, "test_repart_students", id));
  }, [students, teams]);
  ...
  const handleDrop = useCallback(async (e, teamId) => {
    ...
    if (student && student.team !== teamId) {
      ...
      try {
        await addDoc(collection(db, "test_repart_history"), {
      ...
      await updateDoc(doc(db, "test_repart_students", studentId), updateData);
    }
  }, [students, teams]);
  ...
  return (
    <div className="flex flex-col font-sans min-h-screen transition-colors duration-300 bg-brand-bg text-brand-text">
      <header className="sticky top-0 z-[60] border-b border-white/50 p-4 px-8 flex justify-between items-center shadow-soft bg-white/40 backdrop-blur-md">
        <button onClick={() => navigate('/test-stpbb')} className="flex items-center gap-2 transition-colors font-bold text-brand-text/50 hover:text-brand-text"><ArrowLeft size={20} /> Retour</button>
        <h1 className="text-xl font-black tracking-tight flex items-center gap-2 text-brand-text">
          <button onClick={() => navigate('/test-repart')} className="px-4 py-1.5 rounded-full bg-brand-teal/10 text-brand-teal hover:bg-brand-teal hover:text-white transition-all text-xs font-black mr-1" title="Retour Primaire">Primaire</button>
                  ...
                  <textarea
                    ...
                    onBlur={async (e) => {
                      if (e.target.value !== (s.paiComment || "")) {
                        await updateDoc(doc(db, "test_repart_students", s.id), { paiComment: e.target.value });
                      }
                    }}
                  />
                </div>
                  ...
                  <textarea
                    ...
                    onBlur={async (e) => {
                      if (e.target.value !== (s.ebepComment || "")) {
                        await updateDoc(doc(db, "test_repart_students", s.id), { ebepComment: e.target.value });
                      }
                    }}
                  />
                </div>
