...
export default function EdoxiaRepartWrapper() {
  ...
  // Auth state
  const [authRole, setAuthRole] = useState(localStorage.getItem('test_repart_auth') || null);
  ...
  const initializeDatabaseIfNeeded = async () => {
    try {
      const repartSnap = await getDocs(collection(db, "test_repart_students"));
      if (repartSnap.empty) {
        console.log("Initialisation de repart_students depuis students...");
        const studentsSnap = await getDocs(collection(db, "test_students"));
        for (const studentDoc of studentsSnap.docs) {
          await setDoc(doc(db, "test_repart_students", studentDoc.id), {
             ...studentDoc.data(),
  ...
  useEffect(() => {
    ...
    const unsubStudents = onSnapshot(collection(db, "test_repart_students"), (snap) => {
      setStudents(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    // 2. Snapshot for classes
    const qClasses = query(collection(db, "test_repart_classes"), orderBy("numId"));
    const unsubClasses = onSnapshot(qClasses, (snap) => {
      setClassesList(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    // 3. Snapshot for college classes
    const qCollegeClasses = query(collection(db, "test_repart_classes_college"), orderBy("numId"));
