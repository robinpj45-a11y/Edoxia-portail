...
import { GraduationCap, ArrowLeft, Printer, Plus, Search, Lock, Eye, FileText, Flag, User, Users, Trash2, ChevronDown, ChevronUp, Filter, Sparkles } from 'lucide-react';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import { addDoc, updateDoc, deleteDoc, doc, collection } from 'firebase/firestore';
import { db } from '../../firebase';
import { CLASSES } from '../../Edoxia-JS/utils/constants';
  ...
  return (
    <div className="flex flex-col font-sans min-h-screen transition-colors duration-300 bg-brand-bg text-brand-text">
      <header className="sticky top-0 z-[60] border-b border-white/50 p-4 px-8 flex justify-between items-center shadow-soft bg-white/40 backdrop-blur-md">
        ...
        <h1 className="text-xl font-black tracking-tight flex items-center gap-2 text-brand-text"><GraduationCap className="text-indigo-500" /> Répartition 2026-2027</h1>
      </header>
              ...
              <div className="bg-white rounded-[30px] w-full max-w-md flex flex-col shadow-2xl overflow-hidden border border-white/50 animate-in fade-in zoom-in duration-200">
                  ...
                  <form onSubmit={handleAddClass} className="p-6 flex-1 space-y-4 bg-brand-bg">
                      <div className="space-y-1">
                        ...
                        <input type="text" required placeholder="Ex: Futur CP A" className="w-full px-4 py-3 rounded-[16px] focus:outline-none text-sm bg-white/80 border border-white shadow-inner focus:ring-2 focus:ring-indigo-500 text-brand-text font-bold" value={newClassName} onChange={(e) => setNewClassName(e.target.value)} />
                      </div>
                      <button type="submit" className="w-full mt-4 py-4 rounded-[16px] transition-all bg-indigo-500 text-white font-black hover:bg-indigo-600 shadow-soft hover:-translate-y-0.5 active:translate-y-0 flex justify-center items-center gap-2">
                        Créer la classe
                      </button>
                  </form>
              </div>
...
const PersonCard = memo(function PersonCard({ student, onDragStart, onToggle, onDelete }) {
  ...
  return (<div draggable onDragStart={handleLocalDragStart} onDragEnd={handleDragEnd} className={`border border-white/50 rounded-[20px] p-3 shadow-sm cursor-grab active:cursor-grabbing relative hover:shadow-md transition-all group flex justify-between items-start select-none bg-white text-brand-text ${isAdult ? 'border-brand-coral bg-brand-coral/5' : ''}`}><div className="pr-2 overflow-hidden"><div className="font-bold text-sm truncate tracking-tight">{displayName}</div><div className="text-[10px] uppercase text-brand-text/50 mt-0.5 font-bold truncate tracking-wide">{isAdult ? <span className="text-brand-coral">{student.role} • {student.importedClassLabel || student.classLabel}</span> : (student.importedClassLabel || student.classLabel)}</div></div><div className="flex gap-1 shrink-0">{!isAdult && (<><button onClick={() => onToggle(student.id, 'pai')} className={`p-1.5 rounded-full shadow-inner hover:scale-110 active:scale-95 transition-all ${student.pai ? 'text-brand-teal bg-brand-teal/20' : 'text-brand-text/20 bg-black/5 hover:text-brand-teal/80 hover:bg-white'}`} title="Signaler PAI"><FileText size={16} fill={student.pai ? "currentColor" : "none"} /></button><button onClick={() => onToggle(student.id, 'ebep')} className={`p-1.5 rounded-full shadow-inner hover:scale-110 active:scale-95 transition-all ${student.ebep ? 'text-indigo-500 bg-indigo-500/20' : 'text-brand-text/20 bg-black/5 hover:text-indigo-500/80 hover:bg-white'}`} title="Signaler EBEP"><Sparkles size={16} fill={student.ebep ? "currentColor" : "none"} /></button><button onClick={() => onToggle(student.id, 'disruptive')} className={`p-1.5 rounded-full shadow-inner hover:scale-110 active:scale-95 transition-all ${student.disruptive ? 'text-brand-coral bg-brand-coral/20' : 'text-brand-text/20 bg-black/5 hover:text-brand-coral/80 hover:bg-white'}`} title="Élève à surveiller"><Flag size={16} fill={student.disruptive ? "currentColor" : "none"} /></button></>)}<button onClick={() => onDelete(student.id)} className="text-brand-text/30 hover:text-brand-coral opacity-0 group-hover:opacity-100 transition-opacity p-1.5 bg-white shadow-soft rounded-full absolute -top-2 -right-2"><Trash2 size={12} /></button></div></div>);
});
