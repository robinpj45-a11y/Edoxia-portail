  ...
  if (!isAuthenticated) { 
      return (
        <div className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-[#020617]' : 'bg-slate-100'}`}>
            <div className={`p-8 rounded-3xl shadow-xl w-full max-w-md text-center ${isDark ? 'bg-slate-900 border border-slate-800' : 'bg-white'}`}>
                <div className="bg-edoxia-blue w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 text-white"><Settings size={32} /></div>
                <h2 className={`text-2xl font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>Administration</h2>
                <form onSubmit={(e) => { e.preventDefault(); if(password === "stpbb") setIsAuthenticated(true); else alert("Erreur"); }} className="flex gap-2 mt-6">
                    <input type="password" className={`flex-1 p-3 border rounded-xl outline-none focus:border-edoxia-blue ${isDark ? 'bg-slate-950 border-slate-700 text-white' : 'bg-white border-slate-200'}`} autoFocus placeholder="Mot de passe..." value={password} onChange={e => setPassword(e.target.value)} />
                    <button className="bg-edoxia-blue text-white px-6 rounded-xl font-bold hover:bg-blue-700"><LogIn /></button>
                </form>
                <button onClick={() => navigate('/JS2026')} className="mt-4 text-sm text-slate-400 hover:text-slate-600 underline">Retour</button>
            </div>
        </div>
      ); 
  }
    ...
    return (
        <div className="pb-20"><div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">{teams.map(team => {
            ...
            return (
                ...
                <div className="p-4 space-y-2 max-h-[300px] overflow-y-auto">{teamStudents.map(s => (<div key={s.id} className={`text-sm flex justify-between items-center p-2 border-b last:border-0 ${isDark ? 'hover:bg-slate-800 border-slate-800' : 'hover:bg-slate-50 border-slate-100'} ${s.isAdult ? (isDark ? 'bg-red-900/20' : 'bg-red-50') : ''}`}><div className="flex items-center gap-2"><span className={`font-medium ${s.isAdult ? 'text-red-500' : (isDark ? 'text-slate-300' : 'text-slate-700')}`}>{s.name}</span>{!s.isAdult && s.pai && <FileText size={14} className="text-blue-500" />}{!s.isAdult && s.disruptive && <Flag size={14} className="text-red-500 fill-red-500" />}</div><span className={`text-xs px-2 py-0.5 rounded-full ${s.isAdult ? (isDark ? 'bg-red-900/40 text-red-400 font-bold' : 'bg-red-100 text-red-700 font-bold') : (isDark ? 'bg-slate-800 text-slate-400' : 'bg-slate-200 text-slate-600')}`}>{s.isAdult ? s.role : s.classLabel}</span></div>))}</div></div>
