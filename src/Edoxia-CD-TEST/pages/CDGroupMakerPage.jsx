...
export default function CDGroupMakerPage() {
  ...
  useEffect(() => {
    const qStudents = query(collection(db, 'test_cd_students'), orderBy('firstName', 'asc'));
    const unsubscribeStudents = onSnapshot(qStudents, (snapshot) => {
      setStudents(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });

    const qGroups = query(collection(db, 'test_cd_groups'), orderBy('createdAt', 'asc'));
  ...
  const handleCreateGroup = async () => {
    ...
    const docRef = await addDoc(collection(db, 'test_cd_groups'), newGroup);
    setActiveGroupId(docRef.id);
  };

  const handleRenameGroup = async (id, newName) => {
    await updateDoc(doc(db, 'test_cd_groups', id), { name: newName });
  };

  const handleDeleteGroup = async (id) => {
    if (window.confirm("Supprimer ce groupe ?")) {
      await deleteDoc(doc(db, 'test_cd_groups', id));
  ...
  const handleAddStudentToGroup = async (studentId) => {
    ...
    await updateDoc(doc(db, 'test_cd_groups', activeGroupId), {
      studentIds: arrayUnion(studentId)
    });
  };

  const handleRemoveStudentFromGroup = async (groupId, studentId) => {
    await updateDoc(doc(db, 'test_cd_groups', groupId), {
  ...
  return (
    <div className="flex flex-col h-screen bg-brand-bg text-brand-text overflow-hidden">
      <header className="shrink-0 sticky top-0 z-[60] border-b border-white/50 p-4 px-4 md:px-8 flex justify-between items-center shadow-soft bg-white/40 backdrop-blur-md">
        <button onClick={() => navigate('/test-cd')} className="flex items-center gap-2 transition-colors font-bold text-brand-text/50 hover:text-brand-text">
