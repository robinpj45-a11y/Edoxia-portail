...
export default function EdoxiaJSWrapper() {
  ...
  // Auth state
  const [authRole, setAuthRole] = useState(localStorage.getItem('test_js2026_auth') || null);
  ...
  useEffect(() => {
    ...
    const unsubStudents = onSnapshot(collection(db, "test_students"), (snap) => {
      setStudents(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    // 2. Snapshot for teams
    const qTeams = query(collection(db, "test_teams"), orderBy("numId"));
    ...
    const qSlots = query(collection(db, "test_scheduleSlots"), orderBy("order"));
    const unsubSlots = onSnapshot(qSlots, (snap) => {
      setScheduleSlots(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    // 4. Snapshot for schedule activities
    const unsubActivities = onSnapshot(collection(db, "test_scheduleActivities"), (snap) => {
    ...
    const qScores = query(collection(db, "test_scores"), orderBy("createdAt", "desc"));
    const unsubScores = onSnapshot(qScores, (snap) => {
      setScores(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    // 6. Snapshot for bus schedules
    const unsubBus = onSnapshot(collection(db, "test_busSchedules"), (snap) => {
  ...
  const handleLogin = (e) => {
    ...
    if (selectedRole === 'Parent') {
      setAuthRole('Parent');
      localStorage.setItem('test_js2026_auth', 'Parent');
      ...
      localStorage.setItem('test_js2026_auth', 'Formasat');
    } else {
