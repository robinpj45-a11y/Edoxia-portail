...
export default function Teams() {
    ...
    const toggleAppel = async (studentId, currentState) => {
        try {
            await updateDoc(doc(db, "test_students", studentId), { isCalled: !currentState });
        } catch (error) {
    ...
    const resetAppel = async (classLabel) => {
        ...
        try {
            ...
            classStudents.forEach(s => {
                if (s.isCalled) {
                    batch.update(doc(db, "test_students", s.id), { isCalled: false });
                }
            });
            await batch.commit();
        } catch (error) {
        ...
        return (
            <div className="min-h-screen font-sans pb-28 transition-colors duration-300 bg-brand-bg text-brand-text">
                ...
                <nav className="fixed bottom-0 left-0 right-0 h-24 bg-white/90 backdrop-blur-xl shadow-[0_-10px_40px_rgba(0,0,0,0.08)] border-t border-black/5 flex justify-around items-center px-2 pb-safe z-50 rounded-t-[40px]">
                    ...
                    <button onClick={() => navigate('/test-JS2026/search', { state: { fromTeamId: selectedTeamId } })} className="flex flex-col items-center justify-center w-12 sm:w-14 h-14 rounded-2xl gap-1 transition-all text-brand-text/60 hover:bg-black/5">
