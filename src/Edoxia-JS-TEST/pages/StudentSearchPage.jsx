...
export default function StudentSearchPage() {
  ...
  React.useEffect(() => {
    const unsub = onSnapshot(collection(db, "test_student_directory"), (snap) => {
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => {
            if (location.state?.from === 'stpbb') {
              ...
              navigate('/test-JS2026/teams', { state: { fromTeamId: location.state.fromTeamId } });
            } else {
              navigate('/test-JS2026');
            }
