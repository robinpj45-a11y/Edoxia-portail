...
export default function HubPage() {
  ...
  const handleLogout = () => {
    localStorage.removeItem('test_js2026_auth');
    ...
    <div className="min-h-screen flex flex-col font-sans bg-brand-bg text-brand-text transition-colors duration-300">
      <header className="p-4 px-6 rounded-b-[30px] flex justify-between items-center shadow-soft bg-white/40 backdrop-blur-md border-b border-white/50 sticky top-0 z-50">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/test-stpbb')} className="p-2 rounded-full transition-all bg-white/50 hover:bg-white text-brand-text/50 hover:text-brand-text shadow-sm" title="Retour QoL">
            <ArrowLeft size={20} />
          </button>
          <div>
        ...
        <div className="flex gap-3">
          {authRole === 'Ecole' && (
            <button onClick={() => navigate('/test-JS2026/teacher')} className="px-4 py-2 rounded-full flex items-center gap-2 transition-all shadow-soft bg-brand-teal hover:bg-brand-teal/90 text-white hover:scale-105 active:scale-95 font-bold">
      ...
      <main className="p-6 max-w-5xl w-full mx-auto flex-1 flex flex-col pt-8 pb-10">
        <div className="my-auto w-full flex flex-col">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {/* Accessible à tous */}
            <HubCard icon={<Map size={32} className="text-brand-teal" />} title="Carte Interactive" onClick={() => navigate('/test-JS2026/map')} />
            <HubCard icon={<Music size={32} className="text-sky-500" />} title="Chanson JS" onClick={() => navigate('/test-JS2026/song')} />
            <HubCard icon={<Trophy size={32} className="text-yellow-500" />} title="Score en direct" onClick={() => navigate('/test-JS2026/live-scores')} />

            {/* Formasat & Ecole */}
            {(authRole === 'Formasat' || authRole === 'Ecole') && (
              <HubCard icon={<ClipboardList size={32} className="text-indigo-500" />} title="Administrer les scores" onClick={() => navigate('/test-JS2026/scores')} />
            )}

            {/* Uniquement Ecole */}
            {authRole === 'Ecole' && (
              <>
                <HubCard icon={<Users size={32} className="text-brand-peach" />} title="Gérer mon équipe" onClick={() => navigate('/test-JS2026/teams')} />
                <HubCard icon={<Search size={32} className="text-brand-coral" />} title="Rechercher un élève" onClick={() => navigate('/test-JS2026/search')} />
                <HubCard icon={<Settings size={32} className="text-rose-500" />} title="Administration" onClick={() => navigate('/test-JS2026/admin')} />
              </>
            )}
          </div>
